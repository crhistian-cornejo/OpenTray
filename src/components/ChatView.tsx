...
import type { MessageWithParts, Part, TextPart, ToolPart, SessionStatus, AssistantMessage, FullProvider, FullModel } from "../lib/types";
...
type SuggestionMode = "commands" | "models" | null;

export function ChatView({ messages, status, providers, onSendMessage, onAbort }: ChatViewProps) {
  ...
  // Build flattened list of all models
  ...
  // Filter models based on input (when typing /model <search>)
  const filteredModels = useMemo(() => {
    const match = inputMessage.match(/^\/model\s+(.*)$/i);
    if (!match) return [];
    const search = match[1].toLowerCase();
    if (!search) return allModels.slice(0, 20); // Show first 20 when no search
  ...
  useEffect(() => {
    if (inputMessage.toLowerCase().startsWith("/model ")) {
  ...
  }, [inputMessage, filteredCommands.length]);
  ...
  const handleSelectModel = (modelOption: ModelOption) => {
    const command = `/model ${modelOption.model.id}`;
    setInputMessage("");
    setSuggestionMode(null);
    onSendMessage(command);
    inputRef.current?.focus();
  ...
  const currentItems = suggestionMode === "models" ? filteredModels : filteredCommands;
    ...
    <div className="chat fade-in">
      <div className="chat-messages">
        ...
        {messages.length === 0 && (
          <div className="empty">
            <p>No messages yet</p>
            <p className="empty-hint">Type / for commands</p>
          </div>
        )}
      ...
      <div className="chat-input">
        ...
        {suggestionMode === "models" && filteredModels.length > 0 && (
          <div className="command-suggestions model-suggestions">
            <div className="suggestions-header">Select a model</div>
            {filteredModels.map((modelOption, index) => (
              <button
                key={`${modelOption.provider.id}-${modelOption.model.id}`}
                type="button"
                className={`command-item model-item ${index === selectedIndex ? "selected" : ""}`}
                onClick={() => handleSelectModel(modelOption)}
              >
                <span className="model-info">
                  <span className="model-name">{modelOption.model.name}</span>
